...
fvec4 Bitmap::getPixelFromPallete(const unsigned char pixelValue) {
    fvec4 p;
    ...
    if (colorPalleteExists) {
        ...
        for (int l = 0; l < bitmapHeader.BiHeight; l++) {
            ...
            for (int i = 0, j = 0; i <= rowSize - padding - 3; i += (int) (bitmapHeader.BiBitCount / 8), j++) {
                ...
                bitmapArray[l * bitmapHeader.BiWidth + j] = p / 255.0;
            }
        }
    }
...
fvec4 Bitmap::getPixelColorAtPosition(const int l, const int c) const {

    int idx = l * width + c;
    return bitmapArray[idx];
}

fvec2 Bitmap::getPixelPositionOnScreen(const int l, const int c) const {
    ...
    fvec2 pos = {newC + width / 2, newL + height / 2};
    return pos;
}
...
void Bitmap::_nearestNeighbourRotation(const float angle) {
    ...
    fvec4 *newBitmapArray = new fvec4[diagonal * diagonal];
...
void Bitmap::resetImage() {
    ...
    bitmapArray = new fvec4[bitmapHeader.BiHeight * bitmapHeader.BiWidth];
    memcpy(bitmapArray, originalBitmapArray, sizeof(fvec4) * bitmapHeader.BiHeight * bitmapHeader.BiWidth);
...
fvec4 Bitmap::sampleBitmao(const float u, const float v) const {
